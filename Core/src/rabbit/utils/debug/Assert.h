...
namespace RB::Utils::Debug
{
            ...
            #define RB_ASSERT_FATAL_VK(check, ...)          { if((check) != VK_SUCCESS) { RB_LOG_ERROR(RB::LOGTAG_GRAPHICS, __VA_ARGS__); throw std::exception(); } };
        #else
            ...
            #define RB_ASSERT_FATAL_VK(check, ...)          { if((check) != VK_SUCCESS) { RB_LOG_ERROR(__VA_ARGS__); throw std::exception(); } };
        #endif
        ...
        #define RB_ASSERT_FATAL_RELEASE_VK(check, ...)      { if((check) != VK_SUCCESS) { RB_LOG_CRITICAL(RB::LOGTAG_GRAPHICS, __VA_ARGS__); throw std::exception(); } };
    #else
        ...
        #define RB_ASSERT_FATAL_RELEASE_VK(check, ...)      { if((check) != VK_SUCCESS) { RB_LOG_CRITICAL(__VA_ARGS__); throw std::exception(); } };
    #endif
}