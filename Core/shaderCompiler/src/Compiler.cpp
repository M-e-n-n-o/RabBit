...
void Compiler::CompileFiles(std::vector<std::wstring>& files)
{
	for (const std::wstring& file : files)
	{
		...
		for (Shader& entry : entries)
		{
			...
			// The PDB isn't correctly written yet for some reason :(
			static_assert(false);
			
			std::filesystem::create_directories(RB_D3D12_PDB_FOLDER);

			std::wstring pdb_name = L"" RB_D3D12_PDB_FOLDER L"/";
			pdb_name.append(entry.entryName);
